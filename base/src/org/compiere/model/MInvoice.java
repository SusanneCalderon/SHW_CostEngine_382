...
public class MInvoice extends X_C_Invoice implements DocAction
{
	...
	public String completeIt()
	{
		...
		if ((isSOTrx() && getC_Order_ID() != 0)  &&
				getC_Order().getC_DocTypeTarget().getDocSubTypeSO().equals(MDocType.DOCSUBTYPESO_POSOrder) && !fromPOS )
		{
...
//		MOrgInfo counterOrgInfo = MOrgInfo.get(getCtx(), counterAD_Org_ID);
	...
	private String payCashWithCashAsPayment()
	{
        ...
        if (getPaymentRule().equals("K"))
        {
        	paymentCash.setCreditCardNumber(order.get_ValueAsString("CreditCardNumber"));
        	paymentCash.setCreditCardType(order.get_ValueAsString("CreditCardType"));
        	paymentCash.setTenderType(MPayment.TENDERTYPE_CreditCard);
        }
        else  if (getPaymentRule().equals("S"))
        {
        	paymentCash.setTenderType(MPayment.TENDERTYPE_Check);
        	paymentCash.setCheckNo(get_ValueAsString("CheckNo"));
        }
        else
