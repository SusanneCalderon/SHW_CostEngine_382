...
public final class Msg
{
	...
	public static String getAmtInWordsBlankSeparated (Language language, String amount)
	{
		String amtInWords = Msg.getAmtInWords(language, amount);
		StringBuffer amtSep = new StringBuffer("");
		amtInWords.split("");
		for (String part : amtInWords.split(""))
		{
			amtSep.append(part + " ");
		}
		return amtSep.toString();
	}

	public static String getBigDecimalBlankSeperated(BigDecimal amt)
	{
		BigDecimal amtround = amt.setScale(2);
		String amtasString = amtround.toString();
		StringBuffer amtSep = new StringBuffer("");
		amtasString.split("");
		for (String part : amtasString.split(""))
		{
	...
	public static String getDateBlankSeperated(Timestamp date, String format)
	{
		String StringDate = new SimpleDateFormat(format).format(date); 	

		StringBuffer amtSep = new StringBuffer("");
		StringDate.split("");
		for (String part : StringDate.split(""))
		{
			if (part.equals(".") || part.equals(","))
				amtSep.append(part);
			else 
