...
public class MWFActivity extends X_AD_WF_Activity implements Runnable
{
			...
			int AD_Org_ID, boolean ownDocument)
	{
		...
		while (user != null)
		{
			...
			if (role.getSupervisor_ID() != 0)
			{
				...
				return role.getSupervisor_ID();
			}*/
			MRole role = new MRole(getCtx(), Env.getAD_Role_ID(Env.getCtx()), get_TrxName());
			if (user.getSupervisor_ID() != 0)
			{
				user = MUser.get(getCtx(), user.getSupervisor_ID());
				String whereClause = "AD_User_ID=? and ? between " + X_AD_User_Substitute.COLUMNNAME_ValidFrom + " and " 
				+ X_AD_User_Substitute.COLUMNNAME_ValidTo;
				MUser usersubstitute = new Query(getCtx(), X_AD_User_Substitute.Table_Name	, whereClause, get_TrxName())
					.setOnlyActiveRecords(true)
					.setParameters(user.getAD_User_ID(), Env.getContextAsDate(getCtx(), "#Date"))
					.first();
				if (usersubstitute.getAD_User_ID() != 0)
					user = usersubstitute;
				log.fine("Supervisor: " + user.getName()); 
			}

			else if (role.getSupervisor_ID()!= 0)
		...
		if ((getNode().isUserApproval() && getPO(trx) instanceof DocAction))
		{
			...
			try
			{
				...
				if (!"Y".equals(value))
				{
					...
					if (doc.getDocStatus().equals(DocAction.STATUS_Completed))
						newProcess= false;
					if (newProcess)
					{
	...
	public String getSummary()
	{
		...
		if ((keyColumns != null) && (keyColumns.length > 0))
			sb.append(Msg.getElement(getCtx(), keyColumns[0], IssoTrx)).append(" ");
		index = po.get_ColumnIndex("DocumentNo");
		if (index != -1)
			sb.append(po.get_Value(index)).append(": ");

		index = po.get_ColumnIndex("POReference");
		if (index != -1)
			sb.append(po.get_Value(index)).append(": ");/*
		...
		index = po.get_ColumnIndex("TotalLines");
		if (index != -1)
		{
			BigDecimal total = (BigDecimal)po.get_Value(index);
			if (total != null)
			{
		...
		index = po.get_ColumnIndex("shw_reason_Cancelation");
		if (index != -1)
			sb.append("Razon: " + po.get_Value(index)).append(": ");
		return sb.toString();
	}	//	getSummary
