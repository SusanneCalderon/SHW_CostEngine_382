 ...
 */
public class GenerateCostDetail_Autom extends GenerateCostDetailAbstract {
    /**
     * Variables *
     */
    ...
    protected String doIt() throws Exception {
    	String sql = "Select min(dateacct) from m_costdetail where trunc(created) = trunc(getdate())";
    	dateFrom = DB.getSQLValueTSEx(null, sql);
    	dateTo = Env.getContextAsDate(getCtx(), "#Date");
    	if (dateFrom == null)
    		dateFrom = getAccountDate();
        generateCostDetail();
        return "@Ok@";
        ...
        if (MTransaction.MOVEMENTTYPE_VendorReceipts.equals(transaction.getMovementType())) {
            ...
            if (MCostElement.COSTELEMENTTYPE_Material.equals(costElement.getCostElementType())) {
                ...
                orderMatches.stream().forEach(match -> {
                    ...
                    && match.getDateAcct().after(dateFrom)
                    && match.getDateAcct().before(dateTo)) {
            ...
            MLandedCostAllocation.getOfInOutline(line, costElement.getM_CostElement_ID()).stream().forEach(allocation -> {
                if (allocation.getDateAcct().after(dateFrom)
                 && allocation.getDateAcct().before(dateTo))
                CostEngineFactory
    ...
    private void generateCostCollectorNotTransaction(int productId, String trxName)
        ...
        List<MPPCostCollector> costCollectors = MPPCostCollector
                .getCostCollectorNotTransaction(getCtx(), productId, dateFrom	, dateTo, trxName);
