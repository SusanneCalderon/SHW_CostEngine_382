...
public class InfoBPartner extends Info implements PropertyChangeListener, ActionListener, ListSelectionListener
{
	...
	private static Info_Column[] s_Layout = {
		...
		new Info_Column(Msg.translate(Env.getCtx(), "DUNS"), "C_BPartner.DUNS", String.class),
		new Info_Column(Msg.translate(Env.getCtx(), "TaxID"), "C_BPartner.TaxID", String.class)
	};
	...
	//SHW Search for DUNS and TaxID
	private CLabel labelDUNS = new CLabel();
	private CTextField fieldDUNS = new CTextField(10);
	...
	private void statInit()
	{
		...
		//SHW Change email and postal for DUNS ans TaxID
		labelDUNS.setText(Msg.getMsg(Env.getCtx(), "DUNS"));
		fieldDUNS.setBackground(AdempierePLAF.getInfoBackground());
		fieldDUNS.addActionListener(this);

		labelTaxID.setText(Msg.getMsg(Env.getCtx(), "TaxID"));
		fieldTaxID.setBackground(AdempierePLAF.getInfoBackground());
		...
		//SHW Change email and postal for TaxID and DUNS
		//p_criteriaGrid.add(labelEMail, null);
		//p_criteriaGrid.add(fieldEMail, null);
		p_criteriaGrid.add(labelDUNS);
		p_criteriaGrid.add(fieldDUNS);
		//p_criteriaGrid.add(labelPostal, null);
		//p_criteriaGrid.add(fieldPostal, null);
		p_criteriaGrid.add(labelTaxID);
		p_criteriaGrid.add(fieldTaxID);
	...
	protected String getSQLWhere()
	{
		...
		//SHW Search for DUNS and TaxID
		if (isValidSQLText(fieldDUNS))
			list.add ("UPPER(C_BPartner.DUNS) LIKE ?");

		if (isValidSQLText(fieldTaxID))
	...
	protected void setParameters(PreparedStatement pstmt, boolean forCount) throws SQLException
	{
		...
		if (isValidSQLText(fieldTaxID))
		{
			pstmt.setString(index++, getSQLText(fieldTaxID));
			log.fine("TaxID: " + fieldTaxID.getText());
		}
		if (isValidSQLText(fieldDUNS))
		{
			pstmt.setString(index++, getSQLText(fieldDUNS));
			log.fine("DUNS: " + fieldDUNS.getText());
		}
	}   //  setParameters
