		...
		else if (col.equals("M_Product_ID"))
		{
			...
			if(m_mField != null)
			{
				...
				multipleSelection = (MOrderLine.Table_ID ==  AD_Table_ID) || 
									...
									MCommissionLine.Table_ID == AD_Table_ID ||
									MPPProductBOMLine.Table_ID == AD_Table_ID ||
									MRequisitionLine.Table_ID== AD_Table_ID ||
									MCommissionLine.Table_ID == AD_Table_ID ||
									MMovementLine.Table_ID == AD_Table_ID)									
									;
			}
			//	Show Info
		...
		else if (col.equals("C_Invoice_ID"))
		{
			...
			if(m_mField != null)
			{
				...
				multipleSelection  = (MPaymentAllocate.Table_ID ==  AD_Table_ID)	 || 
						(MInvoiceBatchLine.Table_ID == AD_Table_ID)
									;
			}
			//	Show Info
			InfoInvoice ip = new InfoInvoice (frame, true,m_lookup.getWindowNo(), record_id,queryValue,	multipleSelection,true,  whereClause);
			ip.setVisible(true);
			cancelled = ip.isCancelled();
			result = ip.getSelectedKeys();
			resetValue = true;
